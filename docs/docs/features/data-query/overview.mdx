...
sequenceDiagram
    ...
    QE->>QE: Resolve Template Variables ({{limit}}, {{user.id}}, {{args.id}})
...
2.  **Interpolation**: Replacing template variables (e.g., `{{limit}}`, `{{user.id}}`, legacy `{{args.id}}`) with actual runtime values.
...
2.  **`DataQueryTestingForm.jsx`**:
    - The "Run" button.
    - Handles "Prompt for Arguments": If the query needs inputs (e.g., `{{id}}`, `{{user.id}}`, or legacy `{{args.id}}`), it opens a modal (`DataQueryArgsForm`) to ask the user for values before sending the request.
