...
sequenceDiagram
    ...
    QE->>QE: Resolve Template Variables ({{args.limit}}, {{args.user.id}}, {{runtimeArgs.id}})
...
2.  **Interpolation**: Replacing template variables (e.g., `{{args.limit}}`, `{{args.user.id}}`, `{{runtimeArgs.id}}`) with actual runtime values.
...
The **Template Resolution** uses a recursive text replacement strategy with safe object-path lookup:
```javascript
...
2.  **`DataQueryTestingForm.jsx`**:
    - The "Run" button.
    - Handles "Prompt for Arguments": If the query needs inputs (e.g., `{{args.id}}`, `{{args.user.id}}`, or `{{runtimeArgs.id}}`), it opens a modal (`DataQueryArgsForm`) to ask the user for values before sending the request.
