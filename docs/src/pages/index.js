        ...
        <div className="flex flex-col lg:flex-row items-center gap-8">
          <div className="flex-1 space-y-4 text-left">
            ...
            <div className="flex items-center gap-2 pt-2">
              <Link className="h-10 px-5 bg-primary text-primary-foreground font-medium rounded hover:bg-primary/90 transition-all inline-flex items-center justify-center text-sm" to="/docs/introduction">
                Get Started
              </Link>
              <Link className="h-10 px-5 bg-background border border-border text-foreground font-medium rounded hover:bg-muted/50 transition-all inline-flex items-center justify-center text-sm" to="https://github.com/Jet-labs/Jet-admin">
                GitHub
              </Link>
            </div>
          </div>
          <div className="flex-1 w-full max-w-lg lg:max-w-none">
            <div className="rounded border border-border bg-card shadow-md overflow-hidden">
              ...
              <div className="bg-background p-2">
                <img src="img/mockup.png" alt="Jet Admin Dashboard" className="w-full h-auto block rounded border border-border shadow-sm" />
              </div>
            </div>
          </div>
        </div>
...
function FeatureCard({ title, description, icon }) {
  return (
    <div className="rounded border border-border bg-card p-2 flex flex-col gap-2 transition-all hover:border-border/80">
      <div className="bg-muted/30 p-2 rounded border border-border w-10 h-10 flex items-center justify-center">
            ...
            <div className="rounded border border-border bg-card shadow-md overflow-hidden">
              <div className="flex items-center justify-between px-3 py-2 bg-muted/50 border-b border-border">
                ...
                <div className="flex items-center gap-2">
                  <button className="w-6 h-6 flex items-center justify-center bg-background border border-border text-foreground hover:bg-muted/50 rounded transition-colors text-xs cursor-pointer" onClick={prevSlide}>
                    &larr;
                  </button>
                  <button className="w-6 h-6 flex items-center justify-center bg-background border border-border text-foreground hover:bg-muted/50 rounded transition-colors text-xs cursor-pointer" onClick={nextSlide}>
                    &rarr;
                  </button>
                </div>
              </div>
              <div className="relative overflow-hidden w-full h-[300px] md:h-[400px] bg-background">
                {screenshots.map((s, idx) => (
                  ...
                  >
                    <img src={s.image} alt={s.title} className="w-full h-full object-cover rounded border border-border/50" />
                  </div>
                ))}
              </div>
            </div>
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          {testimonials.map((t, i) => (
            <div key={i} className="rounded border border-border bg-card p-2 flex flex-col justify-between gap-4">
              <div className="text-[13px] text-foreground/90 leading-relaxed italic">
                "{t.quote}"
              </div>
              <div className="flex items-center gap-2 pt-2 border-t border-border/50">
                <div className="w-8 h-8 rounded-full bg-muted flex items-center justify-center text-xs font-semibold text-primary">
                  {t.author.split(' ').map(n => n[0]).join('')}
                </div>
                <div>
                  <h4 className="text-xs font-semibold text-foreground leading-none">{t.author}</h4>
                  <p className="text-xs text-muted-foreground mt-1 leading-none">{t.role}</p>
                </div>
              </div>
            </div>
          ))}
      ...
      <div className="max-w-4xl mx-auto px-6 text-center">
        <div className="rounded border border-border bg-card p-4 space-y-4 shadow-sm">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 pt-2">
            <Link to="/docs/introduction" className="h-10 px-5 bg-primary text-primary-foreground font-medium rounded hover:bg-primary/90 transition-all inline-flex items-center justify-center text-sm">
              Get Started Now
            </Link>
            <Link to="https://github.com/Jet-labs/Jet-admin" className="h-10 px-5 bg-background border border-border text-foreground font-medium rounded hover:bg-muted/50 transition-all inline-flex items-center justify-center text-sm">
              View on GitHub
            </Link>
          </div>
        </div>
