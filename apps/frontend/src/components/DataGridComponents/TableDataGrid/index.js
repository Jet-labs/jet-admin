...
export const TableDataGrid = ({
  ...
  useEffect(() => {
    if (selectedRows) {
      _handleMultipleSelectedRowsQueryBuilder(selectedRows);
    }
  }, [selectedRows]);
  ...
  const _handleMultipleSelectedRowsQueryBuilder = useCallback(
    (selectedRows) => {
      ...
      if (_selectedRows?.length == 0) {
        ...
        const multipleSelectedRowsQuery = _selectedRows.map((rowID) => {
  ...
  useEffect(() => {
    console.log({ multipleSelectedQuery });
      ...
      <div item xs={12} className="!relative">
        {isLoadingRows ? (
          ...
          <div className="px-4">
            ...
            <DataGrid
              rows={data.rows}
              columns={[SelectColumn, ...authorizedColumns]}
