...
export const TriggerPolicyEditor = ({ value, handleChange }) => {
  const { themeType } = useThemeValue();
  const [tab, setTab] = useState(
    containsOnly(["add", "delete", "read"], Object.keys(value)) &&
  ...
  return (
    ...
    >
      ...
      >
        <VscGithubAction size={18} className="!text-xs mr-2" />
        {capitalize("Triggers")}
      </span>
      <Divider />
        ...
        {tab === 0 ? (
          <div className="py-2 px-4 ">
            <CRUDPermissionCheckboxGroup
              ...
              permissionKeys={["add", "delete", "read"]}
            />
          </div>
        ) : (
          <div className="p-0">
