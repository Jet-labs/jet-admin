...
export const getTenantAIConfigAPI = async ({ tenantID }) => {
  try {
    const url =
      CONSTANTS.SERVER_HOST +
      CONSTANTS.APIS.TENANT.getTenantAIConfigAPI(tenantID);
    const bearerToken = await firebaseAuth.currentUser.getIdToken();
    if (bearerToken) {
      const response = await axios.get(url, {
        headers: { authorization: `Bearer ${bearerToken}` },
...
export const updateTenantAIConfigAPI = async ({
  tenantID,
  apiKey,
  baseURL,
  provider,
  ...
  try {
    const url =
      CONSTANTS.SERVER_HOST +
      CONSTANTS.APIS.TENANT.updateTenantAIConfigAPI(tenantID);
    const bearerToken = await firebaseAuth.currentUser.getIdToken();
    if (bearerToken) {
      const response = await axios.post(
        url,
