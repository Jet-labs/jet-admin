...
const CustomSuggestionInput = (props) => {
  ...
  return (
    <div className="relative mb-3">
      ...
      <input
        ...
        name={path}
        disabled={!enabled}
        className={`placeholder:text-slate-400 text-sm bg-slate-50 border focus:border-slate-700 ${errors && errors.length > 0
          ? "border-red-500 focus:border-red-500"
          : "border-slate-200"
          } text-slate-700 rounded block w-full px-2.5 py-1.5`}
...
const MyCustomSuggestionInput = withJsonFormsControlProps(CustomSuggestionInput);

const customSuggestionInputTester = rankWith(
  50,
...
export const customJSONFormRenderers = [
  {
    tester: customSuggestionInputTester,
    renderer: MyCustomSuggestionInput,
  },
  {
