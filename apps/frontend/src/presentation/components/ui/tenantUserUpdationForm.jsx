...
import { TenantRoleSelectionDialog } from "./tenantRoleSelectionDialog";
...
export const TenantUserUpdationForm = ({ tenantID, tenantUserID }) => {
  ...
  const {
    ...
    isSuccess: isUpdatingTenantUserRolesSuccess,
    isError: isUpdatingTenantUserRolesError,
    error: updateTenantUserRolesError,
  ...
  const {
    ...
    isSuccess: isRemovingTenantUserFromTenantSuccess,
    isError: isRemovingTenantUserFromTenantError,
    error: removeTenantUserFromTenantError,
  ...
  return tenantUser ? (
    <section class="max-w-3xl w-full h-full">
      <TenantRoleSelectionDialog
        ...
        handleCloseTenantRoleSelectDialog={_handleCloseTenantRoleSelectDialog}
      ...
      <div className="flex flex-col justify-start items-stretch w-full px-3">
        ...
        <span className="text-slate-700 font-medium">{tenantUser.email}</span>
        <span className="!text-slate-600 font-normal text-xs mt-2">
          {tenantUser.isTenantAdmin === CONSTANTS.ROLES.PRIMARY.ADMIN.value
        ...
        <div className="flex flex-row justify-between items-center mt-10 mb-2 w-full">
          <span className="text-slate-700 font-semibold  w-full ">
            {CONSTANTS.STRINGS.UPDATE_TENANT_USER_BY_ID_ASSIGNED_ROLES_TITLE}
          </span>
          <button
        ...
        {tenantUser.isTenantAdmin ? (
          <NoEntityUI
            message={
              CONSTANTS.STRINGS.UPDATE_TENANT_USER_BY_ID_USER_ADMIN_NO_ROLES
            }
          />
        ...
        <div className="flex flex-row justify-end items-center mt-10">
          ...
          >
            {isRemovingTenantUserFromTenant ? (
              <CircularProgress size={16} className="mr-2 !text-[#ff7664]" />
            ) : null}
            {CONSTANTS.STRINGS.UPDATE_TENANT_USER_BY_ID_REMOVE_USER_FROM_TENANT}
          </button>
        </div>
      </div>
    </section>
