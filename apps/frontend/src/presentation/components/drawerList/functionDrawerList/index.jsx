...
import { useFunctionsState } from "../../../../logic/contexts/functionsContext";
import { NoEntityUI } from "../../ui/noEntityUI";
import React from "react";

export const FunctionDrawerList = () => {
  const {
    isLoadingFunctions,
    functions,
    isFetchingFunctions,
  ...
  const _navigateToAddFunction = () => {
    navigate(
      CONSTANTS.ROUTES.ADD_FUNCTION.path(tenantID, databaseSchemaName)
    );
  };
  ...
  return (
    <div className="bg-white h-[calc(100vh-48px)] overflow-hidden p-2 w-full">
      <button
        onClick={_navigateToAddFunction}
      ...
      {isLoadingFunctions || isFetchingFunctions ? (
        ...
        <div className="h-full w-full overflow-y-auto pb-10">
          {functions.map((func) => {
            const key = `function_${func.function_name}`;
            const isActive = routeParam?.functionName === func.function_name;
        ...
        <div className="text-gray-500 dark:text-gray-400">
          <NoEntityUI message="No functions found" />
        </div>
      )}
    </div>
  );
