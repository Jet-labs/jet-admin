import { FaPlus } from "react-icons/fa";
import { MdTableView } from "react-icons/md";
import { Link, useNavigate, useParams } from "react-router-dom";
import { CONSTANTS } from "../../../../constants";
import { useDatabaseViewsState } from "../../../../logic/contexts/databaseViewsContext";
import { NoEntityUI } from "../../ui/noEntityUI";
import React from "react";

export const DatabaseViewDrawerList = () => {
  const {
    isLoadingDatabaseViews,
    databaseViews,
    isFetchingDatabaseViews,
  } = useDatabaseViewsState();
  const routeParam = useParams();
  const { tenantID, databaseSchemaName } = useParams();
  const navigate = useNavigate();

  const _navigateToAddView = () => {
    navigate(
      CONSTANTS.ROUTES.ADD_DATABASE_VIEW.path(tenantID, databaseSchemaName)
    );
  };

  return (
    <div className="bg-white h-[calc(100vh-48px)] overflow-hidden p-2 w-full">
      <button
        onClick={_navigateToAddView}
        className="flex mb-2 flex-row items-center justify-center rounded bg-[#646cff]/10 px-3 py-1.5 text-sm text-[#646cff] hover:bg-[#646cff]/20 focus:ring-2 focus:ring-[#646cff]/50 w-full outline-none focus:outline-none"
      >
        <FaPlus className="!w-4 !h-4 !text-[#646cff] mr-1" />
        Add View
      </button>

      {isLoadingDatabaseViews || isFetchingDatabaseViews ? (
        <div role="status" className="animate-pulse w-full">
          <div className="h-6 bg-gray-200 rounded mb-2 w-full"></div>
          <div className="h-6 bg-gray-200 rounded mb-2 w-full"></div>
          <div className="h-6 bg-gray-200 rounded mb-2 w-full"></div>
          <div className="h-6 bg-gray-200 rounded mb-2 w-full"></div>
        </div>
      ) : databaseViews && databaseViews.length > 0 ? (
        <div className="h-full w-full overflow-y-auto pb-10">
          {databaseViews.map((view) => {
            const key = `databaseView_${view.view_name}`;
            const isActive = routeParam?.databaseViewName === view.view_name;

            return (
              <Link
                to={CONSTANTS.ROUTES.VIEW_DATABASE_VIEW_BY_NAME.path(
                  tenantID,
                  databaseSchemaName,
                  view.view_name
                )}
                key={key}
                className="block mb-2 focus:outline-none"
              >
                <div
                  className={`flex items-center gap-2 px-2 py-1 rounded hover:bg-gray-100 ${
                    isActive ? "bg-[#eaebff]" : "bg-white text-gray-700"
                  }`}
                >
                  <div className="!w-[16px]">
                    <MdTableView
                      className={`w-[16px] h-[16px] ${
                        isActive ? "text-[#646cff]" : "text-slate-600"
                      }`}
                    />
                  </div>
                  <span
                    className={`font-medium text-sm truncate ${
                      isActive ? "font-bold" : ""
                    }`}
                  >
                    {view.view_name}
                  </span>
                </div>
              </Link>
            );
          })}
        </div>
      ) : (
        <div className="text-gray-500 dark:text-gray-400">
          <NoEntityUI message="No views found in this schema" />
        </div>
      )}
    </div>
  );
};
