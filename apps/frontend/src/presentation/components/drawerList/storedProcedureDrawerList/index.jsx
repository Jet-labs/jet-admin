import { FaPlus } from "react-icons/fa";
import { MdSettings } from "react-icons/md";
import { Link, useNavigate, useParams } from "react-router-dom";
import { CONSTANTS } from "../../../../constants";
import { useStoredProceduresState } from "../../../../logic/contexts/storedProceduresContext";
import { NoEntityUI } from "../../ui/noEntityUI";
import React from "react";

export const StoredProcedureDrawerList = () => {
  const {
    isLoadingStoredProcedures,
    storedProcedures,
    isFetchingStoredProcedures,
  } = useStoredProceduresState();
  const routeParam = useParams();
  const { tenantID, databaseSchemaName } = useParams();
  const navigate = useNavigate();

  const _navigateToAddProcedure = () => {
    navigate(
      CONSTANTS.ROUTES.ADD_STORED_PROCEDURE.path(tenantID, databaseSchemaName)
    );
  };

  return (
    <div className="bg-white h-[calc(100vh-48px)] overflow-hidden p-2 w-full">
      <button
        onClick={_navigateToAddProcedure}
        className="flex mb-2 flex-row items-center justify-center rounded bg-[#646cff]/10 px-3 py-1.5 text-sm text-[#646cff] hover:bg-[#646cff]/20 focus:ring-2 focus:ring-[#646cff]/50 w-full outline-none focus:outline-none"
      >
        <FaPlus className="!w-4 !h-4 !text-[#646cff] mr-1" />
        Add Procedure
      </button>

      {isLoadingStoredProcedures || isFetchingStoredProcedures ? (
        <div role="status" className="animate-pulse w-full">
          <div className="h-6 bg-gray-200 rounded mb-2 w-full"></div>
          <div className="h-6 bg-gray-200 rounded mb-2 w-full"></div>
          <div className="h-6 bg-gray-200 rounded mb-2 w-full"></div>
          <div className="h-6 bg-gray-200 rounded mb-2 w-full"></div>
        </div>
      ) : storedProcedures && storedProcedures.length > 0 ? (
        <div className="h-full w-full overflow-y-auto pb-10">
          {storedProcedures.map((proc) => {
            const key = `storedProcedure_${proc.procedure_name}`;
            const isActive = routeParam?.procedureName === proc.procedure_name;

            return (
              <Link
                to={CONSTANTS.ROUTES.VIEW_STORED_PROCEDURE_BY_NAME.path(
                  tenantID,
                  databaseSchemaName,
                  proc.procedure_name
                )}
                key={key}
                className="block mb-2 focus:outline-none"
              >
                <div
                  className={`flex items-center gap-2 px-2 py-1 rounded hover:bg-gray-100 ${
                    isActive ? "bg-[#eaebff]" : "bg-white text-gray-700"
                  }`}
                >
                  <div className="!w-[16px]">
                    <MdSettings
                      className={`w-[16px] h-[16px] ${
                        isActive ? "text-[#646cff]" : "text-slate-600"
                      }`}
                    />
                  </div>
                  <div className="flex flex-col min-w-0">
                    <span
                      className={`font-medium text-sm truncate ${
                        isActive ? "font-bold" : ""
                      }`}
                    >
                      {proc.procedure_name}
                    </span>
                    <span className="text-xs text-slate-400 truncate">
                      procedure • {proc.language}
                    </span>
                  </div>
                </div>
              </Link>
            );
          })}
        </div>
      ) : (
        <div className="text-gray-500 dark:text-gray-400">
          <NoEntityUI message="No procedures found" />
        </div>
      )}
    </div>
  );
};
