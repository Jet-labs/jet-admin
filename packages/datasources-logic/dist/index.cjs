...
var FirestoreDataSource = class extends DataSource {
  ...
  async getFirestoreDb(helpers) {
    ...
    this.db = new import_firestore.Firestore(firestoreConfig);
    return this.db;
  }
...
var MongoDBDataSource = class extends DataSource {
  ...
  async getSchema(params, context) {
    ...
    if (datasourceOptions.connectionString) {
      ...
      if (authSource) params2.append("authSource", authSource);
      if (ssl) params2.append("ssl", "true");
      if (replicaSet) params2.append("replicaSet", replicaSet);
      const queryString = params2.toString() ? `?${params2.toString()}` : "";
      connectionString = `mongodb://${authPart}${host || "localhost"}:${port || 27017}/${dbName}${queryString}`;
    }
    let client;
    try {
      ...
      for (const colInfo of collections) {
        ...
        if (sampleDoc) {
          for (const key of Object.keys(sampleDoc)) {
            ...
            let type = typeof val;
            if (val instanceof import_mongodb.ObjectId) type = "ObjectId";
...
var GoogleSheetsDataSource = class extends DataSource {
  ...
  async getSheetsClient(helpers) {
    ...
    this.sheets = import_googleapis2.google.sheets({ version: "v4", auth });
    return this.sheets;
  }
  async execute(dataQueryOptions2, context, helpers) {
    Logger.log("info", {
...
var WebhookRouter = class {
  constructor() {
    ...
    this._app = (0, import_express.default)();
    this._app.use(import_express.default.json({ limit: "10mb" }));
    this._app.use(import_express.default.urlencoded({ extended: true, limit: "10mb" }));
    this._app.use(import_express.default.raw({ type: "*/*", limit: "10mb" }));
...
var WebSocketDataSource = class extends DataSource {
  ...
  async subscribe(config, onEvent) {
    ...
    const ws = new import_ws2.default(endpoint, protocols || [], options);
    ws.on("message", (data) => {
...
var SSEDataSource = class extends DataSource {
  ...
  async subscribe(config, onEvent) {
    ...
    const es = new import_eventsource.default(endpoint, options);
    es.onmessage = (event) => {
...
var NatsDataSource = class extends DataSource {
  ...
  async subscribe(config, onEvent) {
    ...
    const nc = await (0, import_nats.connect)(options);
...
var firestoreTestConnection = async ({ datasourceOptions, helpers }) => {
  ...
  try {
    ...
    if (finalKey) {
      if (typeof finalKey === "object" && finalKey.refreshToken) {
        try {
          ...
          const oauth2Client = new import_googleapis3.google.auth.OAuth2(clientId, clientSecret);
          oauth2Client.setCredentials({ refresh_token: finalKey.refreshToken });
          firestoreConfig.authClient = oauth2Client;
        } catch (oauthError) {
          Logger.log("error", {
    ...
    const db = new import_firestore2.Firestore(firestoreConfig);
...
var googlesheetsTestConnection = async ({ datasourceOptions, helpers }) => {
  try {
    ...
    if (datasourceOptions.authType === "serviceAccount" && datasourceOptions.serviceAccountKey) {
      ...
      auth = new import_googleapis4.google.auth.GoogleAuth({
        credentials,
      ...
      const oauth2Client = new import_googleapis4.google.auth.OAuth2(clientId, clientSecret);
      oauth2Client.setCredentials({ refresh_token: refreshToken });
      auth = oauth2Client;
    } else {
      return {
    ...
    const sheets = import_googleapis4.google.sheets({ version: "v4", auth });
