...
export const WIDGETS_MAP = {
  ...
  "vega-lite": {
    ...
    datasetFields: [], // Vega-Lite defines fields in the spec
    description: "Advanced composable visualization with Vega-Lite",
    component: ({ data, ...props }) => {
      // Lazy load VegaLiteWidget to avoid bundle bloat for users not using it
      const VegaLiteWidget = React.lazy(() => import("./vega/index"));
      return (
        <React.Suspense fallback={<div>Loading chart...</div>}>
          <VegaLiteWidget data={data} {...props} />
        </React.Suspense>
      );
    },
    icon: ({ className }) => <FaChartBar className={`!text-lg ${className}`} />,
